...
@section('content')
    ...
    <div class="br-mainpanel">
        ...
        <div class="br-pagebody">
            <!-- CONTENT -->
                <form action="/test/{{ $test->id }}" method="post" data-parsley-validated> @csrf @method('put')

                    <div class="br-section-wrapper">
                        <!-- TITLE -->
                        <h5 class="tx-gray-800 mg-b-5">Title </h5>
                        <div class="form-group">
                            <input id="title" name="title" type="text" class="form-control" placeholder="Enter the test title" value="{{ $test->title }}" {{ $enabled }} required>
                        </div><!-- form-group -->

                        <!-- DESCRIPTION -->
                        <h6 class="tx-gray-800 mg-b-5">Description </h6>
                        <div class="form-group">
                            <input id="description" name="description" type="text" class="form-control" placeholder="Enter the test description" value="{{ $test->description }}" {{ $enabled }}>
                        </div><!-- form-group -->

                        <div class="d-flex">
                            <button name="draft" id="draft" type="submit" class="btn btn-dark mr-auto" value="draft" {{ $enabled }}>Draft</button>
                            <button name="publish" id="publish" type="submit" class="btn btn-info mg-r-3" value="publish" {{ $enabled }}>Save</button>
                                ...
                                <tr>
                                    ...
                                    <td class="valign-middle">
                                        <label class="rdiobox mg-b-0">
                                            <input type="radio" name="course" id="course" value="{{ $course->id }}" {{$test->course()->first()->id == $course->id ? "checked":"" }} {{ $enabled }} required><span></span>
                                        </label>
                                    </td>
